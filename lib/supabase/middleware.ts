...
export async function updateSession(request: NextRequest) {
  ...
  // so we skip Supabase entirely (there's no real backend to talk to).
  if (isDemoMode()) {
    const hasDemoSession = request.cookies.get(DEMO_COOKIE)?.value === "1";
    if (hasDemoSession && (path === "/login" || path === "/signup")) {
      const url = request.nextUrl.clone();
      url.pathname = "/process-library";
      url.search = "";
      return NextResponse.redirect(url);
    }
    if (!hasDemoSession && PROTECTED_PREFIXES.some((p) => path.startsWith(p))) {
      const url = request.nextUrl.clone();
      url.pathname = "/login";
